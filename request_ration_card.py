...
def build_encrypted_id(rc_id_plaintext: str, id_type: str, session_id: str) -> str:
    """Replicates CheckAadharSeeding logic for id encryption when idType == "R"."""
    # Default seed (not used when idType == "R"): md5(md5("AePDSImPDs") + "1333")
    seed = md5_hex("AePDSImPDs")
    seed = md5_hex(seed + "1333")

    if id_type == "R":
        # For R: seed = md5(md5("APIMPDS$9712Q") + sessionId)
        seed = md5_hex("APIMPDS$9712Q")
        seed = md5_hex(seed + session_id)

    iv = "AP4123IMPDS@12768F"
    return android_encrypt_double_base64(rc_id_plaintext, seed, iv)


def make_request(rc_id_plaintext: str, id_type: str, state_code: str, session_id: str = None) -> Tuple[int, str]:
    if session_id is None:
        session_id = build_session_id(state_code)

    # For idType "R", use plain text ID (not encrypted)
    # Based on the original request, the id field contains plain ration card ID
    if id_type == "R":
        enc_id = rc_id_plaintext
    else:
        enc_id = build_encrypted_id(rc_id_plaintext, id_type, session_id)
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python request_ration_card.py <rc_id_plaintext> <state_code> <id_type=R> [sessionId]")
        print("Example: python request_ration_card.py 077004047354 28 R")
        sys.exit(1)

    rc_id_plaintext = sys.argv[1]
    state_code = sys.argv[2]
    id_type = sys.argv[3]
    session_id = sys.argv[4] if len(sys.argv) > 4 else None

    status, text = make_request(rc_id_plaintext, id_type, state_code, session_id)
    print(f"HTTP {status}")
    print(text)
